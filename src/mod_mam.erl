...
process_iq(LServer, #jid{luser = LUser} = From, To, IQ, SubEl, Fs, MsgType) ->
	...
	{Start, End, With, RSM} ->
	    ...
	    case MsgType of
		{groupchat, _, #state{room = RoomId, host = RoomHost}} ->
		    #jid{user = UserId, server = UserHost} = From,
		    case mnesia:dirty_read(user_room, {UserId, UserHost, RoomId, RoomHost}) of
			[#user_room{ status = <<"leaved">>, timestamp = Timestamp}] ->
			    select_and_send(LServer, From, To, Start, Timestamp,
